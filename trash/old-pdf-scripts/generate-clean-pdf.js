const puppeteer = require('puppeteer');
const fs = require('fs-extra');
const path = require('path');
const { marked } = require('marked');
const yaml = require('js-yaml');

const inputDir = path.join(__dirname, '..', 'chapters');
const outputDir = path.join(__dirname, '..', 'build', 'dist');
const metadataPath = path.join(__dirname, '..', 'metadata.yaml');

// Create output directory
fs.ensureDirSync(outputDir);

// Load metadata
const metadata = yaml.load(fs.readFileSync(metadataPath, 'utf8'));

// Configure marked to preserve HTML
marked.setOptions({
    breaks: true,
    gfm: true
});

async function generatePDF() {
    const browser = await puppeteer.launch({
        headless: 'new',
        args: ['--no-sandbox', '--disable-setuid-sandbox']
    });

    const page = await browser.newPage();

    // Get all markdown files
    const files = fs.readdirSync(inputDir)
        .filter(file => file.endsWith('.md'))
        .sort();

    let allContent = '';

    // Process each file
    for (const file of files) {
        const filePath = path.join(inputDir, file);
        let content = fs.readFileSync(filePath, 'utf8');
        
        // Extract frontmatter
        const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---/);
        let chapterTitle = '';
        
        if (frontmatterMatch) {
            const frontmatter = yaml.load(frontmatterMatch[1]);
            chapterTitle = frontmatter.title || '';
            content = content.replace(/^---\n[\s\S]*?\n---\n/, '');
        }
        
        // Convert markdown to HTML
        let htmlContent = marked(content);
        
        // Add chapter with page break
        allContent += `
            <div class="chapter">
                ${chapterTitle ? `<h1 class="chapter-title">${chapterTitle}</h1>` : ''}
                ${htmlContent}
            </div>
        `;
    }

    // Create full HTML with ZERO margins for full page usage
    const html = `
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
        <style>
            @page {
                size: 6in 9in;
                margin: 0;
            }
            
            * {
                margin: 0;
                padding: 0;
                box-sizing: border-box;
            }
            
            body {
                font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif;
                font-size: 11pt;
                line-height: 1.65;
                color: #333;
                margin: 0;
                padding: 0.75in;
                width: 6in;
                min-height: 9in;
                background: white;
            }
            
            .chapter {
                page-break-after: always;
                margin-bottom: 0;
            }
            
            .chapter:last-child {
                page-break-after: avoid;
            }
            
            h1, h2, h3, h4, h5, h6 {
                font-weight: 600;
                line-height: 1.2;
                margin-top: 1.5em;
                margin-bottom: 0.5em;
                page-break-after: avoid;
            }
            
            h1 { 
                font-size: 22pt; 
                color: #2c3e50;
                margin-top: 0;
                text-align: center;
            }
            
            h2 { 
                font-size: 18pt; 
                color: #4ECDC4;
                margin-top: 1em;
            }
            
            h3 { 
                font-size: 14pt; 
                color: #34495e;
            }
            
            p {
                margin-bottom: 0.8em;
                text-align: justify;
                hyphens: auto;
            }
            
            ul, ol {
                margin-left: 1.5em;
                margin-bottom: 0.8em;
            }
            
            li {
                margin-bottom: 0.3em;
            }
            
            /* Colorful boxes - Compact */
            .tip-box, .warning-box, .info-box, div[style*="background-color"] {
                padding: 12px 16px;
                margin: 0.8em 0;
                border-radius: 8px;
                page-break-inside: avoid;
                font-size: 10pt;
            }
            
            .tip-box, div[style*="#e8f8f5"] {
                background-color: #e8f8f5 !important;
                border: 2px solid #4ECDC4;
                color: #0a5c55;
            }
            
            .warning-box, div[style*="#fff3cd"] {
                background-color: #fff3cd !important;
                border: 2px solid #FFA06C;
                color: #664d03;
            }
            
            .info-box, div[style*="#e7f3ff"], div[style*="#cfe2ff"] {
                background-color: #e7f3ff !important;
                border: 2px solid #2196F3;
                color: #0d47a1;
            }
            
            /* Code blocks - Very compact */
            pre {
                background-color: #f5f5f5;
                padding: 8px 12px;
                border-radius: 4px;
                overflow-x: auto;
                margin: 0.5em 0;
                font-size: 9pt;
                line-height: 1.3;
                page-break-inside: avoid;
                border: 1px solid #e0e0e0;
            }
            
            code {
                background-color: #f0f0f0;
                padding: 1px 4px;
                border-radius: 3px;
                font-size: 9pt;
                font-family: 'Consolas', 'Monaco', monospace;
            }
            
            pre code {
                background: none;
                padding: 0;
                font-size: 9pt;
            }
            
            /* Tables - Compact */
            table {
                border-collapse: collapse;
                width: 100%;
                margin: 0.8em 0;
                font-size: 10pt;
                page-break-inside: avoid;
            }
            
            th, td {
                border: 1px solid #ddd;
                padding: 6px 10px;
                text-align: left;
            }
            
            th {
                background-color: #f5f5f5;
                font-weight: 600;
            }
            
            /* Blockquotes */
            blockquote {
                border-left: 4px solid #4ECDC4;
                padding-left: 1em;
                margin: 0.8em 0;
                color: #555;
                font-style: italic;
            }
            
            /* Images */
            img {
                max-width: 100%;
                height: auto;
                display: block;
                margin: 0.8em auto;
            }
            
            /* Links */
            a {
                color: #2196F3;
                text-decoration: none;
            }
            
            a:hover {
                text-decoration: underline;
            }
            
            /* Page breaks */
            .page-break {
                page-break-after: always;
            }
            
            /* Reduce spacing for lists inside colored boxes */
            div[style*="background-color"] ul,
            div[style*="background-color"] ol {
                margin-bottom: 0;
                margin-top: 0.3em;
            }
            
            div[style*="background-color"] li {
                margin-bottom: 0.2em;
            }
            
            /* Horizontal rules */
            hr {
                border: none;
                border-top: 1px solid #ddd;
                margin: 1em 0;
            }
            
            /* Definition lists */
            dl {
                margin: 0.8em 0;
            }
            
            dt {
                font-weight: 600;
                margin-top: 0.5em;
            }
            
            dd {
                margin-left: 1.5em;
                margin-bottom: 0.5em;
            }
            
            /* Special styles for specific elements */
            .chapter-title {
                text-align: center;
                margin-bottom: 2em;
            }
            
            /* Keep emojis readable */
            body {
                font-feature-settings: "liga" 1;
            }
        </style>
    </head>
    <body>
        ${allContent}
    </body>
    </html>
    `;

    // Set content and wait for images
    await page.setContent(html, { waitUntil: 'networkidle0' });

    // Generate PDF with NO margins
    const pdfBuffer = await page.pdf({
        path: path.join(outputDir, 'ebook-clean.pdf'),
        format: 'A5', // ~6x9 inches
        printBackground: true,
        margin: {
            top: '0',
            right: '0',
            bottom: '0',
            left: '0'
        },
        displayHeaderFooter: false
    });

    await browser.close();

    console.log('PDF generated successfully!');
    console.log(`Output: ${path.join(outputDir, 'ebook-clean.pdf')}`);
}

// Run the generator
generatePDF().catch(console.error);