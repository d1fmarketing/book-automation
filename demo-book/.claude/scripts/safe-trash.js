...
class SafeTrash {
    ...
    async findInTrash(query) {
        ...
        // Sort by date (newest first)
        results.sort((a, b) => {
            const dateA = a.metadata?.deleted_at || 0;
            const dateB = b.metadata?.deleted_at || 0;
            ...
            case 'search':
                ...
                if (results.length === 0) {
                    ...
                    for (const item of results) {
                        ...
                        console.log(`  Deleted: ${item.metadata ? new Date(item.metadata.deleted_at).toLocaleString() : 'Unknown'}`);
            ...
            case 'info':
                ...
                if (info) {
                    ...
                    if (info.metadata) {
                        ...
                        console.log(`  Deleted At: ${new Date(info.metadata.deleted_at).toLocaleString()}`);
                        console.log(`  Deleted By: ${info.metadata.deleted_by}`);
